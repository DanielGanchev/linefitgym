...
public class UserSpecification implements Specification<UserEntity> {
    ...
    public Predicate toPredicate(Root<UserEntity> root, CriteriaQuery<?> criteriaQuery, CriteriaBuilder criteriaBuilder) {
        if (field.equals("username") || field.equals("email") || field.equals("firstName") || field.equals("lastName") ) {
            ...
            subquery.select(criteriaBuilder.literal("decline"))
                    .where(criteriaBuilder.equal(verificationTokenRoot.get("user"), root),
                            criteriaBuilder.isFalse(root.get("isActive")));

            Expression<String> statusExpression = criteriaBuilder.<String>selectCase()
